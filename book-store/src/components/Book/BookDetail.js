...
const BookDetail = () => {
    ...
    useEffect(() => {
        const fetchHandler = async() => {
            await axios.get(`http://localhost:5001/books/${id}`).then((res)=> res.data).then(data => setInputs(data.book))
        };
        fetchHandler()
    },[id])
  ...
  return (
    ...
    <Button variant='contained' color='primary' type='submit'>Update Book</Button>
    </Box>
</form>
        )}
</div>
